  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      snapshot.forEach(doc => {
        ...
        if (modo === 'semanal') {
          const inicioSemana = data.startOf('isoWeek');
          const fimSemana = data.endOf('isoWeek');
          const chave = `${inicioSemana.format('DD/MM')} a ${fimSemana.format('DD/MM')}`;
          ...
          const chave = data.format('MM/YYYY');
          const diaSemana = data.day();
      ...
      const ultimas = todasChaves
        .sort((a, b) => {
          const parse = modo === 'semanal'
            ? a => dayjs(a.split(' a ')[0], 'DD/MM')
            : a => dayjs('01/' + a, 'DD/MM/YYYY');
      ...
      setDadosGrafico({
        ...
        datasets: [
          ...
          {
            data: quintaData,
            color: (opacity = 1) => `rgba(72, 209, 204, ${opacity})`,
          },
        ],
      ...
      const resumo = ultimas.map(label => {
        const segunda = agrupado[label].segunda;
        const quinta = agrupado[label].quinta;
        return {
          periodo: label,
          segunda,
          quinta,
          total: segunda + quinta,
        };
      });
